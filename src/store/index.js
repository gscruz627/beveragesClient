...
export default createStore({
  ...
  actions: {
    ...
    async registerUser({ commit }, { username, password }) {
      try {
        const userRequest = await fetch("https://beveragesoftheworldapi.onrender.com/API/register", {
          method: "POST",
          headers: {
    ...
    async loginUser({ commit }, { username, password }) {
      try {
        const userRequest = await fetch("https://beveragesoftheworldapi.onrender.com/API/login", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      switch (filter) {
        case "all":
          beveragesRequest = await fetch(
            `https://beveragesoftheworldapi.onrender.com/API/beverages?filter=all`
          );
          break;
        case "country":
          beveragesRequest = await fetch(
            `https://beveragesoftheworldapi.onrender.com/API/beverages?filter=country&country=${country}`
          );
          break;
        case "type":
          beveragesRequest = await fetch(
            `https://beveragesoftheworldapi.onrender.com/API/beverages?filter=type&type=${type}`
          )
      }
    ...
    async addToCart(context, { amount, beverageId, userId, token }) {
      const updateRequest = await fetch(
        `https://beveragesoftheworldapi.onrender.com/API/cart/add/${beverageId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
    ...
    async cleanCart(context, {userId, token}) {
      const updateRequest = await fetch(
        "https://beveragesoftheworldapi.onrender.com/API/cart/clean",{
          method: "PATCH",
          headers: {
            "Content-Type": "application/json"
          },
    ...
    async removeFromCart(context, { beverageId, userId, token }) {
      const updateRequest = await fetch(
        `https://beveragesoftheworldapi.onrender.com/API/cart/remove/${beverageId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
    ...
    async createBeverage(
      ...
      try {
        ...
        const beveragesRequest = await fetch(
          "https://beveragesoftheworldapi.onrender.com/admin/beverages/new",
          {
            method: "POST",
            headers: {
